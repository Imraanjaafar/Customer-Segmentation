...
#%%
csv_path= r"C:\Users\Acer Nitro5\Desktop\Capstone_3\train.csv"
...
#%%
# Instantiate the class with the CSV path
bank_data = BankMarketingData('train.csv')
# Access the DataFrame through the class instance
df = bank_data.data
#%%
...
print(df.describe().T)
#%%
# Extract a specific column (assuming the first column is the desired column)
first_column_name = df.columns[0]
first_column = df[first_column_name].values.reshape(-1, 1)
#%%
# Display information about the extracted column
print(first_column)
...
data = df.copy()
#%%
# Drop the 'last_contact_duration' column
...
for col in ['customer_age', 'marital', 'balance', 'personal_loan', 'last_contact_duration', 'num_contacts_in_campaign']:
...
def remove_outliers(df, col):
    ...
    # Removing outliers above a certain threshold (e.g., 50)
    df = df[df[col] < 50]
...
df = remove_outliers(df, 'num_contacts_in_campaign')
plt.show()
